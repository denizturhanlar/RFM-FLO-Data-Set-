...
#kütüpahaneleri ve gerekli ayarlamaları yapalım
import pandas as pd
...
# csv dosyasını okuttuk ve kopyasını kaydettik
...
# 2. Veri setinde
# İlk 10 gözlem,Değişken isimleri,Boyut,Betimsel istatistik,Boş değer,Değişken tipleri, incelemesi yapınız.
...
df.info()          #Değişken tipleri, incelemesi yaptık
...
#apply(pd.to_datetime) ile tipini değiştiriyoruz
...
# 6. En fazla kazancı getiren ilk 10 müşteriyi sıralayınız.
#sort_values sırala demektir , ascendeing=False yapınca büyükten küçüğe gelir
...
# max() kullanarak son tarihi buluruz son tarih : 2021-05-30 ve genelde son tarihten +2 gün sonrası şeklinde düşünülür
...
rfm["frequency_score"] = pd.qcut(rfm['frequency'].rank(method="first"), 5, labels=[1, 2, 3, 4, 5])  #frequencyde bir ranka sokmamız gerekiyor
...
##################################################
#fonksiyon kullanarakta  yapabilirdik
def rf_score(score):
